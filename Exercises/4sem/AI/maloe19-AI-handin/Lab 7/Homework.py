...
class Variable(object):
    ...
    def calculate_marginal_probability(self):
        ...
        n = 0
        listProbability = {}
        for values in self.probability_table.items():
            valuesOfProbability = values
            listProbability.append(values)
            for entries in valuesOfProbability:
                row += entries
            n += row

            if self.name.lower() =='True':
                secondColumn = values[1]
                firstColumn = values[0]

            if self.name.lower() =='False':
                firstColumn = values[0]
                secondColumn = values[1]
            
            n += secondColumn + firstColumn
            self.marginal_probabilities = [firstColumn/n, secondColumn/n]
...
class BayesianNetwork(object):
    ...
    def get_joint_probability(self, values):
        ...
        dictionary = {}
        for probability_value in values:
            if probability_value in self.varsMap:
                node_variable = self.varsMap[probability_value]
                node_parent = node_variable.get_parents()
                if len(node_parent)>1:
                    for parents in node_parent:
                        index_parent = parents.get_assignment_index(values[parents.get_name()])
                        assignment_parent = parents.get_assignment()
                        for boolean, index in assignment_parent.items():
                            if index == index_parent:
                                assignment = []
                                assignment.append(boolean)
                    joint_probability = list(node_variable.probability_table)
                    number = joint_probability[tuple(assignment)]
                    probability = number[node_variable.get_assignment_index(values[probability_value])]
                    dictionary[node_variable.get_name()] = probability

        results = 1
        for probability_value in dictionary:
            results = results * dictionary[probability_value]
        return results
